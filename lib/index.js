...
exports.register = async function (server, options) {
    ...
    // Link jobs with server
    server.app.scheduler = {};
    server.app.scheduler.schedules = {};
    server.app.scheduler.jobLogs = [];
    server.app.scheduler.jobs = jobs;
    ...
    if (settings.source === Static.Firebase) {
        ...
        // Get schedule ref
        const schedulesRef = server.app.scheduler.db.ref(Static.HapiSchedules);
        ...
        schedulesRef.on('value', (snapshot) => {
            ...
            if (schedules) {
                Object.keys(schedules).forEach((key) => {
                    server.app.scheduler.schedules[key] = {
                        name: schedules[key].name,
                        cronTime: schedules[key].cronTime,
                        requestMethod: schedules[key].requestMethod,
                        requestURL: schedules[key].requestURL,
                        requestHeader: schedules[key].requestHeader,
                        requestBody: schedules[key].requestBody,
    ...
    if (!jobs[key]) {
        jobs[key] = new CronJob({
            ...
            onTick: async function () {
                const jobsRef = FirebaseAdmin.database().ref(Static.HapiJobs);
